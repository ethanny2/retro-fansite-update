...
function checkCollision(){
	...
	console.log('Runner col pt: ' + runnerCollisionPoint + " demon col pt: " + demonCollisionPoint);
	if( (localStorage.getItem('game_loop')==='true') ){
	...
	if(demonCollisionPoint>=runnerCollisionPoint){
		...
		gameOverScreen();
			...
			ajaxScrape();
			stopGame();
			console.log('Local storage game loop is  ---> ' + localStorage.getItem('game_loop'));
...
function instertPhoto(responseObject){
	var image = $('<img class="carti_img slideUp" src="'+responseObject['img_url']+'">');
	$('#main_gallery').append(image);
}

function insertVideo(responseObject){
		var vid = $('<video class="carti_vid slideUp"  onmouseover="this.play()" onmouseout="this.pause()" loop></video>');
		console.log(responseObject['vid_url']);
		var source = $('<source  src="'+responseObject['vid_url']+'"type="video/mp4">');
		vid.append(source);
